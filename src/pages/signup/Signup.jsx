...
function SignUp() {
  ...
  const handleInputPin = (element, index) => {
    ...
    if (checkInputPin.length + 1 === 6) {
      ...
      document
        .querySelector('.btn-submit-pin')
        .classList.replace('bg-[#6457570D]', 'bg-primary');
      document
        .querySelector('.btn-submit-pin')
        .classList.replace('text-[#DADADA]', 'text-white');
    } else {
      document.querySelector('.btn-submit-pin').setAttribute('disabled', true);
      document
        .querySelector('.btn-submit-pin')
        .classList.replace('bg-primary', 'bg-[#6457570D]');
      document
        .querySelector('.btn-submit-pin')
        .classList.replace('text-white', 'text-[#DADADA]');
    }
  };
  ...
  return (
    <main className="container flex flex-row w-screen overflow-x-hidden font-nunito">
          ...
          >
            <div className="flex flex-row border-b-[1.5px] border-[#A9A9A999] py-2 mt-5">
              ...
              <input
                ...
                onChange={(event) => changeHanlder(event)}
              />
            </div>
            <div className="flex flex-row border-b-[1.5px] border-[#A9A9A999] py-2 mt-5">
              <div className="w-[12%] md:w-[8%]">
          ...
          {!isCreatePin ? (
            <section className=" h-screen flex flex-col rounded-[20px] md:rounded-none px-5 md:px-12 py-12 bg-white overflow-hidden">
              ...
              >
                ...
                <button
                  className="btn-submit-pin bg-[#6457570D] text-[#DADADA] rounded-[10px] p-5 cursor-pointer hover:bg-blue-700"
                  disabled
                  type="submit"
                >
                  Confirm
